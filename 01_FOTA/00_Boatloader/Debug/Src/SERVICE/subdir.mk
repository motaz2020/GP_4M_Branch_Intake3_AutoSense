################################################################################
# Automatically-generated file. Do not edit!
# Toolchain: GNU Tools for STM32 (10.3-2021.10)
################################################################################

# Add inputs and outputs from these tool invocations to the build variables 
C_SRCS += \
../Src/SERVICE/SPARSE_program.c 

OBJS += \
./Src/SERVICE/SPARSE_program.o 

C_DEPS += \
./Src/SERVICE/SPARSE_program.d 


# Each subdirectory must supply rules for building sources it contributes
Src/SERVICE/%.o Src/SERVICE/%.su Src/SERVICE/%.cyclo: ../Src/SERVICE/%.c Src/SERVICE/subdir.mk
	arm-none-eabi-gcc "$<" -mcpu=cortex-m4 -std=gnu11 -g3 -DDEBUG -DSTM32F401CCUx -DSTM32 -DSTM32F4 -c -I../Inc -O0 -ffunction-sections -fdata-sections -Wall -fstack-usage -fcyclomatic-complexity -MMD -MP -MF"$(@:%.o=%.d)" -MT"$@" --specs=nano.specs -mfpu=fpv4-sp-d16 -mfloat-abi=hard -mthumb -o "$@"

clean: clean-Src-2f-SERVICE

clean-Src-2f-SERVICE:
	-$(RM) ./Src/SERVICE/SPARSE_program.cyclo ./Src/SERVICE/SPARSE_program.d ./Src/SERVICE/SPARSE_program.o ./Src/SERVICE/SPARSE_program.su

.PHONY: clean-Src-2f-SERVICE

